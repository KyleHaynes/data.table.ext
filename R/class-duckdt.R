...
#' @export
duckdt <- function(conn, table, materialized = NA) {
  ...
  structure(list(conn = conn, tbl = table, materialized = materialized), class = "duckdt")
}
...
print.duckdt <- function(x, n = 6L, ...) {
  ...
  cat(sprintf(
    "<duckdt> %s [%s x %d]%s\n",
    x$tbl, format(nr, big.mark = ","), length(cols),
